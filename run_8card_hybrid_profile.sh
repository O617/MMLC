#!/bin/bash
# Usage: SCHEDULE_MODE=naive|dynamic bash run_8card_hybrid_profile.sh
...
SCHED_MODE=${SCHEDULE_MODE:-dynamic}
echo "Running with SCHEDULE_MODE=${SCHED_MODE}"
...
# 生成带 save_path 的临时 tools.json
MM_TOOL_TEMPLATE='./mindspeed_mm/tools/tools_profile.json'
MM_TOOL="/tmp/tools_profile_hybrid_${SCHED_MODE}.json"
sed "s|PROFILE_SAVE_PATH|./profile_hybrid_${SCHED_MODE}_npu|g" ${MM_TOOL_TEMPLATE} > ${MM_TOOL}

mkdir -p logs
logfile=hybrid_${SCHED_MODE}_profile_$(date +%Y%m%d_%H%M%S)

SCHEDULE_MODE=${SCHED_MODE} torchrun --nproc_per_node $NPUS_PER_NODE --nnodes $NNODES --master_port 30000 \
    pretrain_vlm.py \
