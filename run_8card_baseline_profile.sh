...
MM_TOOL='/tmp/tools_profile_baseline.json'
sed 's|PROFILE_SAVE_PATH|./profile_baseline_npu|g' ${MM_TOOL_TEMPLATE} > ${MM_TOOL}

mkdir -p logs
logfile=baseline_profile_$(date +%Y%m%d_%H%M%S)
